...
public class DailyTop3Keyword {
    public static void main(String[] args) {
        ...
        /**
         * 伪造一份数据，查询文件
         * 备注：实际上，在实际企业项目开发中，这个查询条件，是通过J2EE 平台插入到Mysql中的
         */
        Map<String, List<String>> queryParaMap = new HashMap<>();
        queryParaMap.put("city", Arrays.asList("beijing"));
        queryParaMap.put("platform", Arrays.asList("android"));
        queryParaMap.put("version", Arrays.asList("1.0", "1,2", "1.5", "2.0"));

        //根据实现思路，这里我们需要把查询参数封装成Broadcast广播变量，这样就可以进行优化，每个Worker节点只有一份数据。
        final Broadcast<Map<String, List<String>>> queryParamMapBroadcast = sc.broadcast(queryParaMap);

        // 针对HDFS 文件中的日志，获取输入RDD
        JavaRDD<String> rawRDD = sc.textFile("hdfs://spark1:9000/spark-study/keyword.txt");

        // 使用查询参数Map 广播变量进行筛选数据
        JavaRDD<String> filterRDD = rawRDD.filter(new Function<String, Boolean>() {
            ...
            public Boolean call(String log) throws Exception {
                ...
                //与查询条件进行对比，任何一个条件不满足则返回false, 过滤该日志
        ...
        // 将DataFrame 转换为RDD，然后映射，计算出每天top3 搜索词的搜索uv 总数
        ...
        //再次进行映射，将排序后的数据，映射回原始格式，Iterable<Row>
        ...
        // 将最终的数据，转换为DataFrame 并保存到Hive 表中
